| ID | Expansion | Where defined |
|---|---|---|
| T-*-NN | per-epic Test Contract case | delivery/v4/epics/ |
| C-E*-NN | CONTRACT enforcement case | CONTRACT ## Cases |
| G-n | governance rule | CONTRACT §6 |
| AMD-n | dated amendment to a frozen contract | delivery/decisions.md |
| LG-n | live-grounding fact measured by the coordinator | research/00-main-thread-grounding.md |
| F-*n | research finding by stream | research/stream-*.md |
| D-B<n> | PRD-level design decision (core squad) | delivery/v4/prds/prd-b-core.md |
