...
| `embed` | `path`, `label?` | a `<details class="more">` with the file's markdown in a client-rendered `<script type="text/markdown">`. The embedded markdown's SOURCE participates in G-8 first-use expansion at build time (AMD-12), sharing one page-global first-use set with the rest of the body — outside inline code, fenced blocks, link targets and existing HTML tags; the client renderer keeps the inserted `<abbr>` HTML intact (passthrough) | file not found → `RENDERED_PARTIAL`, visible placeholder "report file not found at build time" |
