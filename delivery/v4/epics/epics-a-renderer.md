...
Types: [unit][integration] · Count: 13 (0 [REAL]) · Surfaces: CLI + HTML-
...
| T-V4A3-13 | AMD-12: Given a manifest whose body is a single `embed` block of a markdown file that mentions the known IDs `T-V4B4-17`, `G-7` and `DoD` — each once in prose, once inside inline code and once inside a fenced block — plus the unknown `Q-99`, preceded by a `copy` block that already mentions `G-7` once; when rendered, then the embed's markdown source in the twin carries exactly one `<abbr class="gl"` expansion for `T-V4B4-17` and for `DoD`, none for `G-7` (already expanded earlier on the page — one page-global first-use set), none inside code spans or fences, `Q-99` warns once; and the template's client-side markdown renderer keeps inline HTML (`<abbr>`) intact — asserted by the presence of its HTML-passthrough setting in the rendered twin | exact counts as stated; exit 2 only for the `Q-99` warning | `node tests/v4/renderer/embed-first-use.mjs` |
